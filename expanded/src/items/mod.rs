...
macro_rules ! impl_wire_item_for_nums { ( $ ( $ ( # [ $ attr : meta ] ) * $ num_ty : ty [ $ bytes : literal ] , ) * ) => { $ ( $ ( # [ $ attr ] ) * impl WireItem for $ num_ty { fn encode < W : Write > ( & self , w : & mut W ) -> std :: io :: Result < usize > { w . write ( &<$ num_ty >:: to_be_bytes ( * self ) ) } fn decode < R : Read > ( r : & mut R ) -> std :: io :: Result < Self > { let mut buf = [ 0_u8 ; $ bytes ] ; r . read_exact ( & mut buf ) ?; Ok ( <$ num_ty >:: from_be_bytes ( buf ) ) } } ) * } ; }
macro_rules ! impl_wire_item_for_byte_array { ( $ ( $ ( # [ $ attr : meta ] ) * [ u8 ; $ bytes : literal ] , ) * ) => { $ ( $ ( # [ $ attr ] ) * impl WireItem for [ u8 ; $ bytes ] { fn encode < W : Write > ( & self , w : & mut W ) -> std :: io :: Result < usize > { w . write ( self ) } fn decode < R : Read > ( r : & mut R ) -> std :: io :: Result < Self > { let mut buf = [ 0_u8 ; $ bytes ] ; r . read_exact ( & mut buf ) ?; Ok ( buf ) } } ) * } ; }
